...
def train(cfg):
    ...
    B_inv = graph["T_basis_inv"].to(device)
    ...
    for epoch in range(cfg["epochs"]):
        ...
        for para, T0, data_id in tqdm(data_loader, ncols=120):
            ...
            f = f_list[0]
            # 边界损失
            loss_con = LossF(T0*mask_BC, f*mask_BC) * f.shape[1] / mask_BC.sum()
            # 求解泰勒系数
            A = LSE(B_inv, adj_mat, f) # [batch, n, n_basis]
            # 微分方程损失
            f_xx = (A*T_xx).sum(dim=2).unsqueeze(2) # [batch, n, 1]
            f_yy = (A*T_yy).sum(dim=2).unsqueeze(2) # [batch, n, 1]
            T_t = (f - T0) / cfg["dt"] # [batch, n, 1]
            loss_PDE = LossF(T_t*mask_iner, para[:,:,0:1]*(f_xx+f_yy)*mask_iner)
        ...
        logs += f"l_c:{l_c/iters:.4e}  l_p:{l_p/iters:.4e} "
        logs += f" n_new:{num_new} | lr:{lr_now:.5e} \n"
        print(logs)
