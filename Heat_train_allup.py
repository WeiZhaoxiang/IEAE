...
def train(cfg):
    ...
    for epoch in range(cfg["epochs"]):
        ...
        l_p = 0
        # 创建
        dataset.full_flag = False #限制迭代次数
        ...
        # 开始迭代
        for para, T0, data_id in tqdm(data_loader, ncols=120, desc="Train:"):
            # 前向传播，自动混合精度
            para = para.to(device)
            T0 = T0.to(device)
            opt.zero_grad()
        ...
        # 更新缓冲区
        n_sam, n_new = dataset.update_all(
            model, 
            coor=coor,
            loss=heat_loss,
            device=device
        ...
        logs += f" update:{n_new} samples:{n_sam} |"
        logs += f" lr:{lr_now:.3e} \n"
        print(logs)
